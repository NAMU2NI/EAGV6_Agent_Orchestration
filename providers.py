...
class OpenAICompatProvider(BaseProvider):
    ...
    def _max_tokens_field(self, model: str) -> str:
        return "max_tokens"
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        body[self._max_tokens_field(m)] = max_tokens
        if tools:
...
def build_providers(cache_store):
    ...
    """
    out = {}
    if k := (os.getenv("OPENAI_API_KEY") or os.getenv("OPEN_AI_API_KEY")):
        out["openai"] = OpenAIProvider(k, os.getenv("OPENAI_MODEL", "gpt-5.1"))
